...
import {
  findExtensionOrganizationById,
  ExtensionOrganizationFormData,
  ExtensionOrganizationStatus,
...
const Settings: React.FC = () => {
  ...
  const populateFormFromRecord = (record: ExtensionOrganizationRecord) => {
  ...
  useEffect(() => {
    const loadRecord = () => {
      ...
      try {
        ...
        if (!session || session.role !== 'Extension Organization') {
          setIsLoadingRecord(false);
          return;
        }
        ...
        if (session.status) {
          setStatus(session.status as ExtensionOrganizationStatus);
        }

        const record = findExtensionOrganizationById(session.id);
        if (record) {
        ...
        console.error('Failed to load extension organization registration', error);
      } finally {
        setIsLoadingRecord(false);
      }
    };
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (!recordId) {
      showNotification('No registration record found. Please complete the Extension Organization registration first.', 'error');
      return;
    }
  ...
  if (formData.areasOfOperation.length === 0) {
    ...
    const notificationId = addNotification({
      ...
      message: `${storedFormData.organizationName || storedFormData.fullName} updated their Extension Organization details for review.`,
      ...
      metadata: {
        ...
        extensionOrganizationId: recordId,
        email: storedFormData.email,
        requiresDecision: true,
      },
    });

    const updatedRecord = updateExtensionOrganizationRecord(recordId, {
  ...
  } as Partial<Omit<ExtensionOrganizationRecord, 'id'>>);
            ...
            <>
              ...
              {!recordId && (
                <p className="text-xs text-yellow-400 font-serif mt-2">
                  No registration data found. Please complete your Extension Organization registration.
                </p>
              )}
            </>
          )}
