...
const Settings: React.FC = () => {
  ...
  const populateFormFromRecord = (record: InsuranceCompanyRecord) => {
  ...
  useEffect(() => {
    const loadRecord = () => {
      ...
      try {
        ...
        if (!session || session.role !== 'Insurance Company') {
          setIsLoadingRecord(false);
          return;
        }
        ...
        if (session.status) {
          setStatus(session.status as InsuranceCompanyStatus);
        }

        const record = findInsuranceCompanyById(session.id);
        if (record) {
        ...
        console.error('Failed to load insurance company registration', error);
      } finally {
        setIsLoadingRecord(false);
      }
    };
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (!recordId) {
      showNotification('No registration record found. Please complete the Insurance Company registration first.', 'error');
      return;
    }
  ...
  if (formData.areasOfOperation.length === 0) {
    ...
    const notificationId = addNotification({
      ...
      message: `${storedFormData.organizationName || storedFormData.fullName} updated their Insurance Company details for review.`,
      ...
      metadata: {
        ...
        insuranceCompanyId: recordId,
        email: storedFormData.email,
        requiresDecision: true,
      },
    });

    const updatedRecord = updateInsuranceCompanyRecord(recordId, {
  ...
  } as Partial<Omit<InsuranceCompanyRecord, 'id'>>);
            ...
            <>
              ...
              {!recordId && (
                <p className="text-xs text-yellow-400 font-serif mt-2">
                  No registration data found. Please complete your Insurance Company registration.
                </p>
              )}
            </>
          )}
