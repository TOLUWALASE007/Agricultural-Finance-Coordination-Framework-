...
const PFIPortal: React.FC = () => {
  ...
  const stats = [
    ...
    { title: 'Anchor Partners', value: '156', change: '+12', icon: '⚓' },
    { title: 'Cooperatives', value: '89', change: '+5', icon: '🤝' }
  ];
  ...
  // Active Schemes search and pagination
  ...
  const filteredActivities = useMemo(() => {
    ...
    if (!q) return activeSchemes;
    return activeSchemes.filter(a =>
      a.type.toLowerCase().includes(q) ||
      a.description.toLowerCase().includes(q) ||
      a.time.toLowerCase().includes(q)
    );
  }, [activitySearch, activeSchemes]);
          ...
          <div className="card">
            ...
            <div className="space-y-4">
              ...
              {paginatedActivities.length === 0 && (
                <div className="text-sm text-gray-400 font-serif">No schemes found.</div>
              )}
            </div>
