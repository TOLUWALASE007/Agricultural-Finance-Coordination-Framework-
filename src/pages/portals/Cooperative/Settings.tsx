...
import {
  findCooperativeGroupById,
  CooperativeGroupFormData,
  CooperativeGroupStatus,
...
const Settings: React.FC = () => {
  ...
  const populateFormFromRecord = (record: CooperativeGroupRecord) => {
  ...
  useEffect(() => {
    const loadRecord = () => {
      ...
      try {
        ...
        if (!session || session.role !== 'Cooperative Group') {
          setIsLoadingRecord(false);
          return;
        }
        ...
        if (session.status) {
          setStatus(session.status as CooperativeGroupStatus);
        }

        const record = findCooperativeGroupById(session.id);
        if (record) {
        ...
        console.error('Failed to load cooperative group registration', error);
      } finally {
        setIsLoadingRecord(false);
      }
    };
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (!recordId) {
      showNotification('No registration record found. Please complete the Cooperative Group registration first.', 'error');
      return;
    }
  ...
  if (formData.areasOfOperation.length === 0) {
    ...
    const notificationId = addNotification({
      ...
      message: `${storedFormData.organizationName || storedFormData.fullName} updated their Cooperative Group details for review.`,
      ...
      metadata: {
        ...
        cooperativeGroupId: recordId,
        email: storedFormData.email,
        requiresDecision: true,
      },
    });

    const updatedRecord = updateCooperativeGroupRecord(recordId, {
  ...
  } as Partial<Omit<CooperativeGroupRecord, 'id'>>);
            ...
            <>
              ...
              {!recordId && (
                <p className="text-xs text-yellow-400 font-serif mt-2">
                  No registration data found. Please complete your Cooperative Group registration.
                </p>
              )}
            </>
          )}
