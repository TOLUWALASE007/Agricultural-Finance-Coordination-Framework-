...
const Settings: React.FC = () => {
  ...
  const populateFormFromRecord = (record: LeadFirmRecord) => {
  ...
  useEffect(() => {
    const loadRecord = () => {
      ...
      try {
        ...
        if (!session || session.role !== 'Lead Firm') {
          setIsLoadingRecord(false);
          return;
        }
        ...
        if (session.status) {
          setStatus(session.status as LeadFirmStatus);
        }

        const record = findLeadFirmById(session.id);
        if (record) {
        ...
        console.error('Failed to load lead firm registration', error);
      } finally {
        setIsLoadingRecord(false);
      }
    };
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (!recordId) {
      showNotification('No registration record found. Please complete the Lead Firm registration first.', 'error');
      return;
    }
  ...
  if (formData.areasOfOperation.length === 0) {
    ...
    const notificationId = addNotification({
      ...
      message: `${storedFormData.organizationName || storedFormData.fullName} updated their Lead Firm details for review.`,
    ...
    const updatedRecord = updateLeadFirmRecord(recordId, {
    ...
    } as Partial<Omit<LeadFirmRecord, 'id'>>);
        ...
        <div className="bg-primary-800 border border-primary-700 rounded-lg p-4">
          ...
          {isLoadingRecord ? (
            ...
            <>
              ...
              No registration data found. Please complete your Lead Firm registration.
                </p>
              )}
            </>
          )}
