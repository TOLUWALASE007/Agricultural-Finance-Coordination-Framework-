...
const Settings: React.FC = () => {
  ...
  const populateFormFromRecord = (record: FundProviderRecord) => {
  ...
  useEffect(() => {
    const loadRecord = () => {
      ...
      try {
        ...
        if (!session || session.role !== 'Fund Provider') {
          setIsLoadingRecord(false);
          return;
        }
        ...
        if (session.status) {
          setStatus(session.status as FundProviderStatus);
        }

        const record = findFundProviderById(session.id);
        if (record) {
        ...
        console.error('Failed to load fund provider registration', error);
      } finally {
        setIsLoadingRecord(false);
      }
    };
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (!recordId) {
      showNotification('No registration record found. Please complete the Fund Provider registration first.', 'error');
      return;
    }
  ...
  if (formData.areasOfOperation.length === 0) {
    ...
    const notificationId = addNotification({
      ...
      message: `${storedFormData.organizationName || storedFormData.fullName} updated their Fund Provider details for review.`,
      ...
      metadata: {
        ...
        fundProviderId: recordId,
        email: storedFormData.email,
        requiresDecision: true,
      },
    });

    const updatedRecord = updateFundProviderRecord(recordId, {
  ...
  } as Partial<Omit<FundProviderRecord, 'id'>>);
            ...
            <>
              ...
              {!recordId && (
                <p className="text-xs text-yellow-400 font-serif mt-2">
                  No registration data found. Please complete your Fund Provider registration.
                </p>
              )}
            </>
          )}
