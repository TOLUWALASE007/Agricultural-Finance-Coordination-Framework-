...
const InsuranceCompanyApplicants: React.FC = () => {
  ...
  // State blocks copied from PFIs pattern
      ...
      <div className="space-y-6">
        <div className="bg-primary-800 rounded-lg p-4 sm:p-6">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-white mb-2">Insurance Company Applicants</h1>
              <p className="text-gray-300">Manage access, restrictions, and approval rights for insurance companies</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-primary-800 rounded-lg p-4 sm:p-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mt-6">
            <p className="text-gray-400 text-xs sm:text-sm">Showing {((approvePage - 1) * pageSize) + 1} to {Math.min(approvePage * pageSize, filteredApproveUsers.length)} of {filteredApproveUsers.length} applicants</p>
            <div className="flex gap-2">
        ...
        <div className="bg-primary-800 rounded-lg p-4 sm:p-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mt-6">
            <p className="text-gray-400 text-xs sm:text-sm">Showing {((editPage - 1) * pageSize) + 1} to {Math.min(editPage * pageSize, filteredEditUsers.length)} of {filteredEditUsers.length} users</p>
            <div className="flex gap-2">
        ...
        <div className="bg-primary-800 rounded-lg p-4 sm:p-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mt-6">
            <p className="text-gray-400 text-xs sm:text-sm">Showing {((restrictPage - 1) * pageSize) + 1} to {Math.min(restrictPage * pageSize, filteredRestrictUsers.length)} of {filteredRestrictUsers.length} users</p>
            <div className="flex gap-2">
        ...
        <div className="bg-primary-800 rounded-lg p-4 sm:p-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mt-6">
            <p className="text-gray-400 text-xs sm:text-sm">Showing {((approvalRightsPage - 1) * pageSize) + 1} to {Math.min(approvalRightsPage * pageSize, filteredApprovalRightsUsers.length)} of {filteredApprovalRightsUsers.length} users</p>
            <div className="flex gap-2">
            ...
            <div className="fixed inset-0 z-50 bg-black/60 p-4 overflow-y-auto" onClick={() => setShowApprovalRightsMoreInfo(null)}>
              <div className="min-h-screen flex items-center justify-center py-8">
                <div className="w-full max-w-2xl bg-primary-900 rounded-lg border border-primary-700 p-6" onClick={(e) => e.stopPropagation()}>
                  ...
                  <div className="flex justify-end mt-4">
                    <button onClick={() => setShowApprovalRightsMoreInfo(null)} className="btn-primary">Close</button>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="fixed inset-0 z-50 bg-black/60 p-4 overflow-y-auto" onClick={() => setShowApprovalHistory(false)}>
            <div className="min-h-screen flex items-center justify-center py-8">
              <div className="w-full max-w-3xl bg-primary-900 rounded-lg border border-primary-700 p-6" onClick={(e) => e.stopPropagation()}>
                <div className="flex items-start justify-between mb-4">
                  <h3 className="text-lg font-semibold font-sans text-gray-100">Approve Access History</h3>
                  <button onClick={() => setShowApprovalHistory(false)} className="text-gray-400 hover:text-gray-200">✖</button>
                </div>
                <div className="space-y-3">
                  {[
                    { date: '2024-10-15 14:30', action: 'Approved', user: 'Leadway Assurance', by: 'Admin User', remarks: 'All documents verified' },
                    { date: '2024-10-10 09:15', action: 'Rejected', user: 'AIICO Insurance', by: 'Admin User', remarks: 'Incomplete documentation' },
                    { date: '2024-10-05 16:45', action: 'Approved', user: 'NEM Insurance', by: 'Admin User', remarks: 'Compliance check passed' }
                  ].map((entry, idx) => (
                ...
                <div className="flex justify-end mt-4">
                  <button onClick={() => setShowApprovalHistory(false)} className="btn-primary">Close</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="fixed inset-0 z-50 bg-black/60 p-4 overflow-y-auto" onClick={() => setShowRestrictHistory(false)}>
            <div className="min-h-screen flex items-center justify-center py-8">
              <div className="w-full max-w-3xl bg-primary-900 rounded-lg border border-primary-700 p-6" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="space-y-3">
                  {[
                    { date: '2024-10-13 13:45', user: 'NEM Insurance', reason: 'Non-compliance', by: 'Admin User', remarks: 'Failed compliance audit' },
                    { date: '2024-10-08 09:30', user: 'AIICO Insurance', reason: 'Incomplete documents', by: 'Admin User', remarks: 'Missing required documentation' },
                    { date: '2024-10-03 14:20', user: 'Leadway Assurance', reason: 'Other', by: 'Admin User', remarks: 'Temporary restriction pending review' }
                  ].map((entry, idx) => (
                    <div key={idx} className="bg-primary-800 rounded p-3">
                      <div className="flex justify-between items-start mb-2">
                        <span className="text-sm text-gray-400">{entry.date}</span>
                ...
                <div className="flex justify-end mt-4">
                  <button onClick={() => setShowRestrictHistory(false)} className="btn-primary">Close</button>
                </div>
              </div>
            </div>
          </div>
